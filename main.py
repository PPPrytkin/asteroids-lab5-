...
class Ship:
    ...
    def shoot(self):
        ...
        missile_group.add(cl.Sprite(missile_pos, missile_vel, self.angle, 0, missile_image, missile_info, missile_sound))
...
def group_collide(group,other_object):
    ...
    for element in list(group):
        if element.collide(other_object):
            explosion_group.add(cl.Sprite([element.get_position()[0] - 3*element.radius,element.get_position()[1] - 3*element.radius], [0,0], 0, 0, explosion_images, explosion_info,explosion_sound))
...
def draw(canvas):
    ...
    if st.started:
        ...
        canvas.blit(splash, (st.WIDTH//2 - 200, st.HEIGHT//2 - 150))

    """Отображение жизней"""
    myfont1 = pygame.font.SysFont("Comic Sans MS", 20)
    label1 = myfont1.render("Lives : "+str(st.lives), 1, (255,255,0))
    canvas.blit(label1, (50,20))

    """Отображение счёта"""
    myfont2 = pygame.font.SysFont("Comic Sans MS", 20)
    label2 = myfont2.render("Score : "+str(st.score), 1, (255,255,0))
...
def click(event):
    ...
    if event.button == 1 and event.pos[0] in range(st.WIDTH//2 - 200, st.WIDTH//2 + 200) and event.pos[1] in range(st.HEIGHT//2 - 150, st.HEIGHT//2 + 150) and not st.started:
        st.started = True
